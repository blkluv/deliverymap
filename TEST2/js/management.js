...
function renderAreaOnMap(mapId, areaBounds) {
    ...
    try {
        ...
        const layer = new ol.layer.Vector({ source, style: (f) => f.get('style') });
        ...
        if (boundsData.v === 1 && boundsData.o) {
            ...
            const { p: palette, c: compressedCells } = boundsData;
            const markerReverseMap = { 'e': 'entrance', 'x': 'exit', 't': 'table', 'p': 'parking' };

            compressedCells.forEach(cellStr => {
                const [coords, fillIdx, markerChar, markerColorIdx] = cellStr.split(':');
                const [x, y] = coords.split(',').map(Number);
                const lon = centerLonLat[0] + x * 0.000033;
                const lat = centerLonLat[1] + y * 0.000033;
                const extent = ol.proj.transformExtent([lon, lat, lon + 0.000033, lat + 0.000033], 'EPSG:4326', 'EPSG:3857');
                
                const feature = new ol.Feature({ geometry: ol.geom.Polygon.fromExtent(extent) });

                const styles = [];
                const fillColor = fillIdx !== '' ? palette.f[parseInt(fillIdx, 10)] : null;
                if (fillColor) {
                    styles.push(new ol.style.Style({ fill: new ol.style.Fill({ color: fillColor }) }));
                }

                const marker = markerChar !== '' ? markerReverseMap[markerChar] : null;
                if(marker) {
                    const markerColor = markerColorIdx !== '' ? palette.m[parseInt(markerColorIdx, 10)] : '#000';
